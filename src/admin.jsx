    ...
    <div className="mt-30 flex min-h-screen bg-gray-100">
      ...
      <main className="flex-1 p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[{
            icon: <FaUsers size={40} />, title: "All Users", color: "from-red-400 to-red-600"
          }, {
            icon: <FaFileInvoiceDollar size={40} />, title: "All Submitted Tax", color: "from-green-400 to-green-600"
          }, {
            icon: <FaRegListAlt size={40} />, title: "User Form Fields", color: "from-blue-400 to-blue-600"
          }].map(({ icon, title, color }, index) => (
            <div key={index} className={`bg-gradient-to-r ${color} p-6 rounded-lg shadow-lg text-white flex flex-col items-center transform hover:scale-105 transition duration-300`}>
              {icon}
              <h2 className="text-lg font-semibold mt-3">{title}</h2>
              <button className="mt-3 bg-white text-gray-900 px-4 py-2 rounded shadow-md hover:bg-gray-200 transition">
                Click To See
              </button>
            </div>
          ))}
