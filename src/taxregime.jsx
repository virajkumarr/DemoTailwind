const TaxSlabTable = () => {
  const taxData = [
    ...
    { income: "", rate: "15%", newIncome: "Above ₹24 lakh", newRate: "15%" }
  ];

  return (
    <div className="mt-35 p-6 max-w-4xl mx-auto bg-gray-300 shadow-lg rounded-lg">
      ...
      <table className="w-full border-collapse text-left rounded-lg overflow-hidden shadow-md">
        ...
        <tbody>
          {taxData.map((row, index) => (
            <tr key={index} className={`border border-gray-300 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-100'}`}> 
      ...
      <p className="mt-4 text-sm italic text-gray-700 text-center">*Total Annual Income after excluding standard deduction of Rs 75,000</p>
    </div>
  );
};
